...
`LANFileServer` 的外层交付目录直接放一键入口脚本、生成好的 `.dmg` 和内部项目目录，不需要把脚本再用同名文件夹包起来。

本文里的路径都以当前 `README.md` 所在目录为基准：`./` 就是这个外层交付目录。把整套目录移动到其它位置后，文档里的路径仍然通用。
...
4. 选中左侧项目，点击顶部启动按钮。
5. 把顶部蓝色访问地址发送给同一局域网内的其它电脑或手机。
6. 右侧访问记录会显示访问时间、访问人 IP、服务引擎、状态码、对象和路径。

- 左侧项目支持右键菜单，可以执行启动 / 停止、Show in Finder、移除。
